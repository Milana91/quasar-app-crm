      ...
      <q-card-section class="q-pt-md">
        <q-form ref="resetPasswordForm">
          <q-input
            ...
            :rules="[
              (val) =>
                (!!val && val.length > 0) || 'Пожалуйста, введите свой email',
            ]"
          />
        </q-form>
...
export default {
  ...
  setup() {
    ...
    const timer = setInterval(() => {
        ...
        if (seconds > 0) {
          dialog.update({
            message: `Окно будет закрыто автоматически через ${seconds} секунд${
              seconds > 1 ? "у" : ""
            }.`,
          });
        } else {
    ...
    const resetPassword = function () {
      sendPasswordResetEmail(auth, form.email)
        .then(() => {
          ...
          $q.notify({
            message:
              "Ссылка для генерации нового пароля отправлена на ваш почтовый ящик",
          });
